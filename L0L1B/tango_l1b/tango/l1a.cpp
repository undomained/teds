    ...
    if (ckd->lev > LEVEL_STRAYCAL) {
        ...
        if (il1x_start < L1X_STRAY && opt.stray) { // {{{
            ...
            if (!ckd->stray_skip && ckd->stray.n_kernels == 0) {
                ...
                const int d_rows { static_cast<int>(ckd->dim_detector_spat) };
                const int d_cols { static_cast<int>(ckd->dim_detector_spec) };
                const int detector_size { d_rows*d_cols };
                const int tr_r { static_cast<int>(ckd->stray_transformed_n_rows) };
                const int tr_c { static_cast<int>(ckd->stray_transformed_n_cols) };
                const int padding { (tr_r - d_rows)/2 };
                const int detector_transformed_size { tr_r*tr_c };

                // Van Cittert algorithm
                //
                //               frame_cur - K x frame_ideal
                // frame_ideal = ---------------------------,
                //                        1 - eta
                //
                // where frame_cur is the current image being processed, K is a
                // stray light kernel (either the constant or moving part),
                // frame_ideal is frame_cur with the stray light contribution
                // removed ("ideal" image), eta is the internal scattering
                // factor (must be small for the algorithm to converge,
                // typically a few percent), and x denotes convolution.
                ...
                for (int i {}; i < opt.stray_van_cittert_steps; ++i) {
                    ...
                    // Ghost kernels
                    // Transform current frame for convolution with a
                    // moving kernel
                    const auto &tr_indices { ckd->stray_transform_indices };
                    const auto &tr_deltas { ckd->stray_transform_deltas };
                    int delta_idx { -1 };
                    for (int i {}; i < detector_transformed_size; ++i) {
                        const int idx { tr_indices[i] };
                        auto &val { frame_transformed[i] };
                        val = tr_deltas[++delta_idx]*frame_ideal[idx];
                        val += tr_deltas[++delta_idx]*frame_ideal[idx+d_cols];
                        val += tr_deltas[++delta_idx]*frame_ideal[idx+1];
                        val += tr_deltas[++delta_idx]*frame_ideal[idx+d_cols+1];
                    }
                    vector<double> tmp(detector_transformed_size);
                    convolve_fft(
                        tr_r,
                        tr_c,
                        frame_transformed,
                    ...
                    for (int i {}; i < d_rows; ++i) {
                        for (int j {}; j < d_cols; ++j) {
                            const auto idx { (i+padding)*tr_c };
                            conv[i*d_cols+j] += tmp[idx + j+padding];
                        }
                    }
                    for (int i {}; i < detector_size; ++i) {
                        frame_ideal[i] =
                          (image[i] - conv[i])/(1 - ckd->stray_eta);
                    }
                }
                for (int i {}; i < detector_size; ++i) {
