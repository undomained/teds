...
class ISRF(Algorithm):
    ...
    def apply_isrf(self,input_data):
        ...
        g_weights = np.zeros((n_det_cols, wavelength.shape[0], wavelength.shape[1]))
        weights = np.zeros((n_det_cols, wavelength.shape[0], wavelength.shape[1]))
        def apply_gauss(x):
        ...
        for i_act in range(wavelength.shape[0]):
            ...
            delta_lambda = values[2:] - values[:-2]
        ...
        for i_det in range(n_det_cols):
            ...
            g_weights[i_det,:,:] = apply_gauss(wavelength)
            weights[i_det, :, :]  = np.multiply(g_weights[i_det,:,:], delta_lambdas)
            conv_signal = np.multiply(weights[i_det, :, :], image)
