...
)
{
    ...
    if (lev_target > LEVEL_DARKCAL) { // {{{
        ...
        if (!dark_skip) {
            ...
            handle(read_opt(&nc_in,grp,opt_dark));
    ...
    if (lev_target > LEVEL_NOISECAL) { // {{{
        ...
        if (!noise_skip) {
            ...
            handle(read_opt(&nc_in,grp,opt_noise));
    ...
    if (lev_target > LEVEL_NONLINCAL) { // {{{
        ...
        if (!nonlin_skip) {
            ...
            handle(read_opt(&nc_in,grp,opt_nonlin));
    ...
    if (lev_target > LEVEL_PRNUCAL) { // {{{
        ...
        if (!prnu_skip) {
            ...
            handle(read_opt(&nc_in,grp,opt_prnu));
        }
    ...
    if (lev_target > LEVEL_FOVCAL) { // {{{
        ...
        fov_ispat.resize(dim_fov*DIM_POL*dim_detector_spec); // Contents of the file to be further processed.
        fov_ipix1.resize(dim_fov*DIM_POL*dim_detector_spec); // Unbinned processed CKD.
        fov_ipix2.resize(dim_fov*DIM_POL*dim_detector_spec); // Unbinned processed CKD.
        fov_weight1.resize(dim_fov*DIM_POL*dim_detector_spec); // Unbinned processed CKD.
        ...
        for (size_t ifovpol=0 ; ifovpol<dim_fov*DIM_POL ; ifovpol++) {
        ...
        for (size_t ifov=0 ; ifov<dim_fov ; ifov++) fov_iel_start[ifov] = ifov*DIM_POL*dim_detector_spec; // This is always true for unbinned CKD.

        handle(read_opt(&nc_in,grp,opt_fov));
    ...
    if (lev_target > LEVEL_SWATHCAL) { // {{{
        ...
        if (!swath_skip) {
            ...
            handle(read_opt(&nc_in,grp,opt_swath));
        }
    ...
    if (lev_target > LEVEL_WAVECAL) { // {{{
        ...
        wave_spectra.resize(dim_fov*DIM_POL*dim_detector_spec);
        ...
        handle(read_opt(&nc_in,grp,opt_wave));
    ...
    if (lev_target > LEVEL_RADCAL) { // {{{
        ...
        if (!rad_skip) {
            ...
            rad_spectra.resize(dim_fov*DIM_POL*dim_detector_spec);
            netcdf_check(&nc_in,grp.getVar("rad_spectra").getVar(rad_spectra.data()));

            handle(read_opt(&nc_in,grp,opt_rad));
