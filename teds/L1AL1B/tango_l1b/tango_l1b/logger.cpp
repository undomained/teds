...
Logger::Logger( // {{{
  ...
  : inactive { creator->inactive },
    my_rank { creator-> my_rank },
    num_procs { creator->num_procs }
{
...
)
{
    ...
    if (inactive || level == log_verbose && !verboseflag) return;
...
auto Logger::setInactive(const bool inactive) -> void
{
