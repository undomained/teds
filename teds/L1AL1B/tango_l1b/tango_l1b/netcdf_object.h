...
private:
    bool opened = false;
    bool writeaccess;
    static auto copyAtts(const netCDF::NcVar& in, netCDF::NcVar& out) -> void;
public:
    string filename; // Public because it is needed inside a macro.
    unique_ptr<NcFile> ncid;
    NetCDF_object(Logger *creator);
    ~NetCDF_object();
    int open(string &a_filename, NcFile::FileMode fmode);
    static auto copyVar(const netCDF::NcVar& var,
