...
def readsgmatmosphere(sgmatmosphere_file, instrument):
    ...
    if(instrument == 'TANGO_Carbon'):
        data.__setattr__("CO2_column", f["col_co2"][:])
        data.__setattr__("CH4_column", f["col_ch4"][:])
        data.__setattr__("H2O_column", f["col_h2o"][:])
...
def simplified_level2(sgmatmosphere_inputfile, level2_outputfile, instrument, precision_rel, precision_const, seed=10):
    ...
    level2_outputfile : String
        Output file name of the Level 2 data.
    instrument : String Instrument TANGO-Carbon or TANGO-Nitro
    precision_rel : Float, relative error
    precision_const : Float, constant error
    seed : Int, Optional
        A seed to generate random numbers for level 2 precision.

    """
    # read data for a given gas
    data = readsgmatmosphere(sgmatmosphere_inputfile, instrument)
    nalt, nact, nlay = data.zlay.shape
    ...
    if(instrument=='TANGO_Carbon'):
        data.__setattr__("precision_XCO2_proxy", data.CO2_column*precision_rel + precision_const)
        data.__setattr__("precision_XCH4_proxy", data.CH4_column*precision_rel + precision_const)
        data.__setattr__("XCO2_proxy", getlevel2retrieval(data.CO2_column, data.precision_XCO2_proxy, seed))
        data.__setattr__("XCH4_proxy", getlevel2retrieval(data.CH4_column, data.precision_XCH4_proxy, seed))
    ...
    writedata_l2data(level2_outputfile, data, instrument)
