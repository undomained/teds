...
def level2_output(filename, l2product, retrieval_init, l1bproduct, settings):
    ...
    _ = writevariablefromname(output_l2,
    ...
    _ = writevariablefromname(
        output_l2, 'aqui_time',       _dimalt, l2_aqutime)
    # processing flag
    _ = writevariablefromname(
        grp_diag,  'process_flag',    _dims, l2_proc_flag)
    # convergence
    _ = writevariablefromname(
        grp_diag,  'convergence',     _dims, l2_conv)
    # latitude
    _ = writevariablefromname(
        output_l2, 'latitude',        _dims, l1bproduct['latitude'][:, :nact])
    # longitude
    _ = writevariablefromname(
        output_l2, 'longitude',       _dims, l1bproduct['longitude'][:, :nact])
    # maxiterations
    _ = writevariablefromname(
        grp_diag,  'maxiterations',   _dims, l2_numb_iter)
    # chi2
    _ = writevariablefromname(grp_diag,  'spectralchi2',    _dims, l2_chi2)
    # albedo
    _ = writevariablefromname(output_l2, 'albedo',          _dims, l2_alb)
    # surface pressure
    _ = writevariablefromname(grp_prior,
                              'surface_pressure',
                              _dims,
                              retrieval_init['surface pressure'])
    # surface elevation
    _ = writevariablefromname(grp_prior,
                              'surface_elevation',
                              _dims,
                              retrieval_init['surface elevation'])
    # spectral shift
    _ = writevariablefromname(
        output_l2, 'spectralshift',   _dims, l2_spec_shift)
    # chi2
    _ = writevariablefromname(
        output_l2, 'spectralsqueeze', _dims, l2_spec_squeeze)
    ...
    for gas in gases:
        ...
        for ialt in range(nalt):
            for iact in range(nact):
                ...
                l2_X[ialt, iact] = tmp
                l2_X_prec[ialt, iact] = (
                    l2product[ialt, iact][xgas_precision] * scale[gas])
                l2_X_avgk[ialt, iact, :] = l2product[ialt, iact][xgas_ker][:]
                l2_X_prof[ialt, iact, :] = (
                    retrieval_init['trace gases'][gas]['ref_profile'])
        ...
        _ = writevariablefromname(
            output_l2, avgker_varname, _dims3d, l2_X_avgk)
        _ = writevariablefromname(
            grp_prior, aprof_varname,  _dims3d, l2_X_prof)
...
def level2_diags_output(filename, l2product, measurement):
    # This function writes some ectra diagnostics to a file that are
    # not included in the level 2 data product.
    nalt = len(l2product)
    nact = len(l2product[0])
    ...
    # spectral axis
    output_l2diag.createDimension('bins_spectral', nwave)
    # across track axis
    output_l2diag.createDimension('bins_along_track', nalt)
    # across track axis
    output_l2diag.createDimension('bins_across_track', nact)
    ...
    for ialt in range(nalt):
        for iact in range(nact):
            ...
            l2diag_wave[ialt, iact, :nw] = (
                measurement[ialt, iact]['wavelength'][:])
            l2diag_measurement[ialt, iact, :nw] = (
                measurement[ialt, iact]['ymeas'][:])
            l2diag_sun_irradiance[ialt, iact, :nw] = (
                measurement[ialt, iact]['sun'][:])
            l2diag_gainCO2[ialt, iact, :nw] = (
                l2product[ialt, iact]['gain_XCO2'][:])
            l2diag_gainH2O[ialt, iact, :nw] = (
                l2product[ialt, iact]['gain_XH2O'][:])
            l2diag_gainCH4[ialt, iact, :nw] = (
                l2product[ialt, iact]['gain_XCH4'][:])

    _dims = ('bins_along_track', 'bins_across_track', 'bins_spectral')
    # wavelength of measurement
    _ = writevariablefromname(
        output_l2diag, "l2wavelength", _dims, l2diag_wave)
    # spectral radiance measurements
    _ = writevariablefromname(
        output_l2diag, "l2measurement", _dims, l2diag_measurement)
    # solar irradiance spectrum
    _ = writevariablefromname(
        output_l2diag, "l2solarirradiance", _dims, l2diag_sun_irradiance)
    # gain CO2
    _ = writevariablefromname(
        output_l2diag, "l2gainCO2", _dims, l2diag_gainCO2)
    # gain CH4
    _ = writevariablefromname(
        output_l2diag, "l2gainCH4", _dims, l2diag_gainCH4)
    # gain H2O
    _ = writevariablefromname(
        output_l2diag, "l2gainH2O", _dims, l2diag_gainH2O)
    output_l2diag.close()
...
def level2_output_RTorCH4(
    ...
    for vartup in extra_variables:
        varname, pargroup, vardata, vartype = (None, None, None, 'f8')
        if len(vartup) == 4:
...
def level1b_to_level2_processor_RTorCH4(config):
    ...
    with torch.no_grad():
        ...
        for batch in range(N_alt):
            ...
            for chunk, chunk_inds in enumerate(act_chunks):
                ...
                for i in range(N_iter):
                    ...
                    out_chi2[batch, chunk_inds, i] = (
                        chi2.cpu().detach().numpy())
        ...
        for spi, spn in enumerate(species_names):
            ...
            l2[f'avgkernelX{spn}'] = (
                out_col_kern_base[:, :, spi, :] * delta_prof)
...
def level2_nan(retrieval_init, nlay):
    ...
    for _, key in enumerate(retrieval_init['trace gases'].keys()):
