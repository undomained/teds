...
def ifdoe_run(config, mode='no2'):
    ...
    parameterNames = IFDOEparameters(cfg)
    ...
    if cfg['irr_from_sgm']:
        irrWvl, irr, irrError, irrFlag = ReadIrrSGM(cfg['io']['sgm_irr'], pxlN)
    else:
        # TODO: not implemented yet
        sys.exit()
        # irrWvl, irr, irrError, irrFlag = ReadIrr(cfg['irr_file'], pxlN)
    ...
    if cfg['wlcal_solar']:
        irrWvlScaled = ScaleWvl(irrWvlCal,cfg['fit_window'])
    else:
        irrWvlScaled = ScaleWvl(irrWvl,cfg['fit_window'])
  ...
  # C.1  Set up arrays for the scanline results
    ...
    def process_scanline(iscan,results):
        ...
        if cfg['rad_from_sgm']:
            radWvlScan, radScan, radErrorScan, radFlagScan = ReadRadSGM(ncRad, iscan)
        else:
            radWvlScan, radScan, radErrorScan, radFlagScan = ReadRad(ncRad, iscan)
        ...
        for ipxl in range(pxlBeg,pxlEnd+1,1):
            ...
            if cfg['wlcal_earth']:
                radWvlScaled = ScaleWvl(radWvlCal,cfg['fit_window'])
            else:
                radWvlScaled = ScaleWvl(radWvl,cfg['fit_window'])

            # E)  Reflectance
            # ---------------
            # # E.1  Interpolate solar grid to earth grid using ref_solar
            # irrRegridded, irrErrorRegridded = SolarToEarthWvl(IFDOERefSpec,radWvlScaled,irrWvlScaled,irr,irrError,ipxl)

            # # E.1  Interpolate solar grid to earth grid
            irrRegridded, irrErrorRegridded = SolarToEarthWvl_simple(radWvlScaled,irrWvlScaled,irr,irrError,ipxl)
            ...
            model = IFDOEmodel(prior=prior, priorCov=priorCov, 
                otherModelParam=None, 
            ...
            # F.2  Perform fit
            try:
                oe = OE(model=model, maxiter=cfg['max_iterations'], stateVectorConvThreshold=cfg['convergence_threshold'])
            ...
            # F.3  Spike removal -> re-do E.2, but only once
                # detect outliers   
                wvl = ScaleWvl(commonWvlFit,cfg['fit_window'])
        ...
        # F.4  Determine diagnostics: RMS error, degrees of freedom, etc.
            ...
            wvl = DeScaleWvl(commonWvlFit,cfg['fit_window'])
