...
def readgridtime(f, data, gas):
    ...
    if "x" in f.variables.keys(): # vars x and y not always in microHH data
...
def readmeteodata(path_data, gasestoread, filesuffix):
    data = DataCont()
    # read individual gas concentrations and grids
    readgrid = True   # flag to read the grid only once
    for gas in gasestoread:
        # check if the file exists
        filename = path_data + gas + "_" + filesuffix + ".nc"
        if (Path(filename)).is_file():
            # read file
            f = Dataset(filename, "r")
            # read grid data
            if readgrid:
                readgridtime(f, data, gas.upper())
                readgrid = False
            # read concentrations
...
def readmeteovelocitydata(path_data, filesuffix):
    data = DataCont()
    # read velocities
    # check if the file exists
    filename = path_data + "velocity_" + filesuffix + ".nc"
    if (Path(filename)).is_file():
        # read file
        f = Dataset(filename, "r")
        # read grid data
        readgridtime(f, data, 'u')
        # read u, v, w
        data.__setattr__('u', f['u'][0])  # '0' because it has time component
        data.__setattr__('v', f['v'][0])
        data.__setattr__('w', f['w'][0])
        f.close()
    else:
        print(filename, " file doesn't exist")
        sys.exit()
...
def _get_default_files(dir_data):
    ...
    List of strings
        List of file names
    """
    default_files = list(Path(dir_data).glob("*default*"))
    return default_files.sort()
...
def _get_ini_file(dir_data):
    ...
    return (list(Path(dir_data).glob("*.ini")))[0]
