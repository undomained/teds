# -*- coding: utf-8 -*-
"""
Created on Mon Sep  5 14:57:49 2022
...
class CloudmaskS2(object):
    ...
    def __init__(self, path, aux_path, cartopy_path_directory, clear=False):
        ...
        self.varname_cloud = [
            'cloud_reff', 'cloud_optical_thickness', 'cloud_top_height_asl', 'cloud_phase', 'cloud_fraction'
        ]

    def read_file(self):

        _logger.info("Reading cloud file over Berlin area {}.".format(self.path + "/" + self.filename))
    ...
    def collocate(self, julday_orbit, lat, lon, n_pixels, use_watermask=True):
        """Given time and lat-lon box of the groundpixel, return the collocated cloud"""
        ...
        if use_watermask:
            ...
            for i in range(0, len(self.lat_orbit)):
                if (self.is_land(self.lon_orbit[i], self.lat_orbit[i]) is False) or \
                        (self.is_lake(self.lon_orbit[i], self.lat_orbit[i]) is True) or \
                        (self.is_river(self.lon_orbit[i], self.lat_orbit[i]) is True):
        ...
        if self.cloud_mask_in is None:
            self.read_file()
        self.cloudmask_orbit, idx_cloud, idx_clear, self.original_cloud_fraction = collocate_cloudmask(self.lat_orbit, self.lon_orbit, n_pixels,
                                                                                                       self.lats, self.lons, self.cloud_mask_in,
                                                                                                       fraction_minimum=0.05, lonlat_gridded=False)
    ...
    def post_process(self, albedo_data):
        ...
        tot_albedo = self.original_cloud_fraction * ((10 * (0.14)) / ((10 * (0.14)) + 2)) + (1 - self.original_cloud_fraction) * albedo_data_band560

        self.cloud_optical_thick_orbit[:] = ((-2 * tot_albedo) / ((tot_albedo - 1) * 0.14))

    def write_output(self, output_dir):

        output_file = output_dir + 'sentinel_cloudmask.nc'

        write_pixel_parameter_to_nc(self.cloudmask_orbit, 'cloudmask', self.lat_orbit, self.lon_orbit, self.npixels, output_file)
    ...
    def write_variables_to_group(self, group, var_selection, start=0, end=None):

        if 'cloud_top_height_asl' in var_selection:
            group.variables['cloud_top_height_asl'][start:end] = self.cloud_top_height_asl_orbit[:]
        ...
        if 'cloud_fraction' in var_selection:
            ...
            group.variables['cloud_fraction'][start:end] = self.cloud_fraction_orbit[:]

        if 'original_cloud_fraction' in var_selection:
            group.variables['original_cloud_fraction'][start:end] = self.original_cloud_fraction[:]

    def prepare_shapes(self, path):
        try:
            import shapefile
            shapefiles_path = self.cartopy_path_directory + '/shapefiles'
            land_shp_fname = shapefiles_path + '/natural_earth/physical/ne_50m_land.shp'
            lake_shp_fname = shapefiles_path + '/natural_earth/physical/ne_110m_lakes.shp'
            river_shp_fname = shapefiles_path + '/natural_earth/physical/ne_110m_rivers_lake_centerlines.shp'
            land_sf = shapefile.Reader(land_shp_fname)
            lake_sf = shapefile.Reader(lake_shp_fname)
            river_sf = shapefile.Reader(river_shp_fname)

            land_geom = unary_union([shape(land_sf.shape(rec.oid).__geo_interface__)
                                     for rec in land_sf.iterRecords() if rec["featurecla"] != "Null island"])
            lake_geom = unary_union([shape(lake_sf.shape(rec.oid).__geo_interface__)
                                     for rec in lake_sf.iterRecords() if "Lake" in rec["featurecla"]])
            river_geom = unary_union([shape(river_sf.shape(rec.oid).__geo_interface__)
                                      for rec in river_sf.iterRecords() if "River" in rec["featurecla"]])

        except ImportError:
            import cartopy
            import cartopy.io.shapereader as shpreader
            cartopy.config['pre_existing_data_dir'] = self.cartopy_path_directory
            ...
            lake_shp_fname = shpreader.natural_earth(resolution='110m', category='physical', name='lakes')

            lake_geom = unary_union([record.geometry
                                     for record in shpreader.Reader(lake_shp_fname).records()
                                     if "Lake" in record.attributes.get('featurecla')])

            river_shp_fname = shpreader.natural_earth(resolution='110m', category='physical', name='rivers_lake_centerlines')

            river_geom = unary_union([record.geometry
