...
hybrid_a = np.array([0.0e+00, 4.804826e-02, 6.593752e+00, 1.313480e+01,
                     1.961311e+01, 2.609201e+01,
                     3.257081e+01, 3.898201e+01, 4.533901e+01, 5.169611e+01,
                     5.805321e+01, 6.436264e+01,
                     7.062198e+01, 7.883422e+01, 8.909992e+01, 9.936521e+01,
                     1.091817e+02, 1.189586e+02,
                     1.286959e+02, 1.429100e+02, 1.562600e+02, 1.696090e+02,
                     1.816190e+02, 1.930970e+02,
                     2.032590e+02, 2.121500e+02, 2.187760e+02, 2.238980e+02,
                     2.243630e+02, 2.168650e+02,
                     2.011920e+02, 1.769300e+02, 1.503930e+02, 1.278370e+02,
                     1.086630e+02, 9.236572e+01,
                     7.851231e+01, 5.638791e+01, 4.017541e+01, 2.836781e+01,
                     1.979160e+01, 9.292942e+00,
                     4.076571e+00, 1.650790e+00, 6.167791e-01, 2.113490e-01,
                     6.600001e-02, 1.000000e-02,
                     0.000000e+00])  # [hpa]

hybrid_a = np.flip(hybrid_a)  # now from bottom to top

hybrid_b = np.array([1.000000e+00, 9.849520e-01, 9.634060e-01, 9.418650e-01,
                     9.203870e-01, 8.989080e-01,
                     8.774290e-01, 8.560180e-01, 8.346609e-01, 8.133039e-01,
                     7.919469e-01, 7.706375e-01,
                     7.493782e-01, 7.211660e-01, 6.858999e-01, 6.506349e-01,
                     6.158184e-01, 5.810415e-01,
                     5.463042e-01, 4.945902e-01, 4.437402e-01, 3.928911e-01,
                     3.433811e-01, 2.944031e-01,
                     2.467411e-01, 2.003501e-01, 1.562241e-01, 1.136021e-01,
                     6.372006e-02, 2.801004e-02,
                     6.960025e-03, 8.175413e-09, 0.000000e+00, 0.000000e+00,
...
class WRFCO2M(object):
    ...
    def read_file(self):
        ...
        # latitude, northern degree (ntime, nlat, nlon)
        self.lat_in = data["lat"]
        self.lon_in = data["lon"]  # longitude, degree east (ntime, nlat, nlon)
        # fraction of surface pressure, with ptop at 50hPa (ntime, nlev+1)
        eta = data["eta"]
        ...
        for mlat in range(self.nlat):
            for mlon in range(self.nlon):
                press[:, mlat, mlon] = (
                    spress[self.mtime, mlat, mlon] * eta[self.mtime, :])

        # From Pa to HPa
        press = press * 1.E-2
        # see decription of upper pressure boundary in netcdf file
        press[0, :, :] = 50
        ...
        co2_afgl = atm_afgl[:, 7]/air_afgl

        # To expand the wrf profile, we define two indices index1 and
        # index2. index1 indicates the layer where afgl and wrf
        # profiles are clued together. Between index1 and index2 we
        # scale the afgl to have a smooth transition at level
        # index1. Above index2, the scaling depends on layer index
        # such that at TOA we obtain the original afgl value.  Here,
        # index2 is chosen such that afgl mixing ratio is nearly
        # constant at altitudes below.
        ...
        for mlat in range(self.nlat):
            for mlon in range(self.nlon):
                ...
                self.press_final[:, mlat, mlon] = np.append(
                    press_afgl[:index1], press[:, mlat, mlon])
                self.co2_final[:, mlat, mlon] = np.append(
                    co2_afgl[:index1]*1.E6, co2[self.mtime, :, mlat, mlon])

                # define a scaling to get a smooth transition between
                # afgl and wrf
                fact = (co2[self.mtime, 0, mlat, mlon]
                        / co2_afgl[index1+1]*1.E-6)-1

                scale[index1:] = 1.              # range of the wrf profile
                # constant scaling of afgl profile for smooth extension of wrf
                scale[index2:index1] = 1.+fact
                # to get upper value of afgl at TOA
                scale[:index2] = 1. + fact*np.arange(index2)/float(index2)

                self.co2_final[:, mlat, mlon] = scale * self.co2_final[
                    :, mlat, mlon]

                # calculate dry air column mixing ratio

                # air column [#/m^2] above pressure level
                vc_air = sp * self.press_final[:, mlat, mlon]

                self.xco2_in[mlat, mlon] = (
                    np.sum((vc_air[1:] - vc_air[0:vc_air.size-1])
                           * self.co2_final[:, mlat, mlon])
                    / vc_air[vc_air.size-1])
    ...
    def collocate(self, julday_orbit, lat_orbit, lon_orbit, n_pixels):
        ...
        self.xco2 = interpolate_single_parameter_to_orbit(
            lat_orbit,
            lon_orbit,
            n_pixels,
            self.lat_in[self.mtime, :, :],
            self.lon_in[self.mtime, :, :],
            self.xco2_in,
            lonlat_gridded=True,
            method='linear')

        self.press_final_orbit = interpolate_to_orbit(
            lat_orbit,
            lon_orbit,
            n_pixels,
            self.lat_in[self.mtime, :, :],
        ...
        self.co2_final_orbit = interpolate_to_orbit(
            lat_orbit,
            lon_orbit,
            n_pixels,
            self.lat_in[self.mtime, :, :],
    ...
    def write_to_group(self, group, start=0, end=None):
        ...
        group['CMR_co2'].long_name = (
            "dry air column mixing ratio total, top down")
...
class CAMSEGGCO2M(object):
    ...
    def read_file(self, julday):
        ...
        # [ppm] dry air
        self.tcco2_cams = f_r.variables['tcco2'][idx_time][:][:]
        # [ppb] dry air
        self.tcch4_cams = f_r.variables['tcch4'][idx_time][:][:]
    ...
    def collocate(self, julday_orbit, lat, lon, npixels):
        ...
        # nearest_linear.
        self.tcco2_orbit = interpolate_single_parameter_to_orbit(
            lat, lon, npixels, self.lat_cams, self.lon_cams, self.tcco2_cams)
        self.tcch4_orbit = interpolate_single_parameter_to_orbit(
            lat, lon, npixels, self.lat_cams, self.lon_cams, self.tcch4_cams)
...
class CAMSEAC4CO2M(object):
    ...
    def collocate(self, julday_orbit, lat, lon, npixels):
        ...
        self.tcno2_orbit = interpolate_single_parameter_to_orbit(
            lat, lon, npixels, self.lat_cams, self.lon_cams, self.tcno2_cams)

        self.tco3_orbit = interpolate_single_parameter_to_orbit(
            lat, lon, npixels, self.lat_cams, self.lon_cams, self.tco3_cams)

        self.tch2o_orbit = interpolate_single_parameter_to_orbit(
            lat, lon, npixels, self.lat_cams, self.lon_cams, self.tch2o_cams)
...
class CO2ME2EAUX(object):
    ...
    def collocate(self, julday_orbit, lat_orbit, lon_orbit, n_pixels):
        ...
        if self.include_wrf:
            ...
            self.camsEGGref.collocate(
                self.wrf.julday * np.ones(julday_orbit.shape),
                lat_orbit,
                lon_orbit,
                n_pixels)
    ...
    def post_process(self, n_pixels, psurf, pixel_altitude):
        ...
        f = interp.interp1d(
            np.log(self.p_in), self.z_in, fill_value='extrapolate')
        ...
        f = interp.interp1d(
            np.log(self.p_in), self.t_in, fill_value='extrapolate')
        ...
        for ipix in range(n_pixels):
            ...
            zlev_tmp[1:nlev - 1] = 0.5 * (zlay[ipix, 0:nlev - 2]
                                          + zlay[ipix, 1:nlev - 1])
            zlev_tmp[0] = 2 * zlev_tmp[1] - zlev_tmp[2]
            zlev_tmp[nlev - 1] = 2 * zlev_tmp[nlev - 2] - zlev_tmp[nlev - 3]
            dtemp = (
                (tlev_tmp[1:nlev] - tlev_tmp[0:nlev - 1])
                / (zlev_tmp[1:nlev] - zlev_tmp[0:nlev - 1]))
        ...
        f = interp.interp1d(
            np.log(self.p_in), self.xo3_in, fill_value='extrapolate')
        xo3 = np.zeros((n_pixels, nlay))
        for ilay in range(nlay):
            xo3[:, ilay] = f(np.log(play[:, ilay]))

        f = interp.interp1d(
            np.log(self.p_in), self.xh2o_in, fill_value='extrapolate')
        xh2o = np.zeros((n_pixels, nlay))
        for ilay in range(nlay):
            xh2o[:, ilay] = f(np.log(play[:, ilay]))

        f = interp.interp1d(
            np.log(self.p_in), self.xco2_in, fill_value='extrapolate')
        ...
        if self.include_wrf:
            ...
            for lay in range(self.wrf.nlev):
                press_center[lay, :] = 0.5 * (
                    self.wrf.press_final_orbit[lay, :]
                    + self.wrf.press_final_orbit[lay + 1, :])
            for pid in range(n_pixels):
                f = interp.interp1d(np.log(press_center[:, pid]),
                                    self.wrf.co2_final_orbit[:, pid],
                                    fill_value='extrapolate')
                # Here the data will have the same shape as the rest
                # of the data.
                for ilay in range(nlay):
                    wrfxco2[pid, ilay] = f(np.log(play[pid, ilay]))

        f = interp.interp1d(
            np.log(self.p_in), self.xno2_in, fill_value='extrapolate')
        xno2 = np.zeros((n_pixels, nlay))
        for ilay in range(nlay):
            xno2[:, ilay] = f(np.log(play[:, ilay]))

        f = interp.interp1d(
            np.log(self.p_in), self.xch4_in, fill_value='extrapolate')
        ...
        for ilay in range(nlay):
            # Mair/Mwater=1.60855
            dvair[:, ilay] = (
                (plev[:, ilay + 1] - plev[:, ilay]) * avoga * 1.e-2 /
                (air_m * grav * (1. + xh2o[:, ilay] / 1.60855)))
        ...
        _logger.debug("XO3 scaling min/max {}, {}".format(np.min(scale),
                                                          np.max(scale)))
        ...
        _logger.debug("H2O scaling min/max {}, {}".format(np.min(scale),
                                                          np.max(scale)))
        ...
        _logger.debug("NO2 scaling min/max {}, {}".format(np.min(scale),
                                                          np.max(scale)))
        ...
        _logger.debug("CO2 scaling min/max {}, {}".format(np.min(scale),
                                                          np.max(scale)))
        ...
        if self.include_wrf:
            ...
            _logger.debug("WRF CO2 scaling min/max {}, {}".format(
                np.min(scale), np.max(scale)))
        ...
        _logger.debug("CH4 scaling min/max {}, {}".format(np.min(scale),
                                                          np.max(scale)))
        ...
        # dry MMR to specific humidity (moist MMR)
        self.xh2o = xh2o / (1 + xh2o)

    def write_output(self, output_dir):
        if self.psurf is None:
            raise ProcessError("Post processing step, which needs aerosol "
                               "echam variables not executed.")
