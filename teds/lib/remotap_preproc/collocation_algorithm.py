...
def interpolate_single_parameter_to_orbit(lat_orbit, lon_orbit, n_pixels, lat_in, lon_in, parameters_in, lonlat_gridded=False, method='nearest_linear'):
    ...
    if (len(x1) > 0):
        ...
        parameter_orbit_tmp[:] = sc.interpolate.griddata((x1, y1), newarr, (lon_orbit.flatten(), lat_orbit.flatten()), method='nearest')
        t_inter = time.perf_counter()
        _logger.debug("Nearest interpolation done. Linear following.")
        parameter_orbit[:] = sc.interpolate.griddata((x1, y1), newarr, (lon_orbit.flatten(), lat_orbit.flatten()), method='linear')

        if method == 'nearest':
            parameter_orbit[:] = sc.interpolate.griddata((x1, y1), newarr, (lon_orbit.flatten(), lat_orbit.flatten()),
                                                         method='nearest')
        if method == "nearest_linear":

            parameter_orbit_tmp[:] = sc.interpolate.griddata((x1, y1), newarr, (lon_orbit.flatten(), lat_orbit.flatten()),
                                                             method='nearest')
    ...
    if method == 'nearest_linear':
        _logger.debug("Time for interpolations: nearest {}, linear {}, total {}".format(t_inter - t_start, t_end - t_inter, t_end - t_start))
    else:
        _logger.debug("Time for interpolations: nearest {}".format(t_end - t_start))

    return parameter_orbit


def interpolate_to_orbit(lat_orbit, lon_orbit, n_pixels, lat_in, lon_in, parameters_in, n_parameters, transpose=False, lonlat_gridded=False,
                         keep_minmax=True, method='nearest_linear'):
    ...
    for ipar in range(0, n_parameters):
        ...
        if len(x1) > 0:
            ...
            if method == 'nearest':
                orbit_data[ipar, :] = sc.interpolate.griddata((x1, y1), newarr, (lon_orbit.flatten(), lat_orbit.flatten()),
                                                              method='nearest')
            if method == "nearest_linear":

                orbit_data_tmp[ipar, :] = sc.interpolate.griddata((x1, y1), newarr, (lon_orbit.flatten(), lat_orbit.flatten()),
                                                                  method='nearest')
            ...
            if method == 'nearest_linear':
                ...
                _logger.debug("Time for interpolations: {} {}".format(method, t_end - t_start))

    return orbit_data


def collocate_cloudmask(lat_orbit, lon_orbit, n_pixels, lat_in, lon_in, cloudmask_in, fraction_minimum=0.5, lonlat_gridded=True):
    ...
    # interpolation to orbit data for Cloud parameters. Outside range, set to one, cloudy
    ...
    if (len(x1) > 0):
        ...
        cloud_mask_orbit[:] = sc.interpolate.griddata((x1, y1), newarr, (lon_orbit, lat_orbit), method='nearest')
        indices = np.where((lat_orbit < np.min(lat_in)) | (lat_orbit > np.max(lat_in)) |
                           (lon_orbit < np.min(lon_in)) | (lon_orbit > np.max(lon_in)))
        cloud_mask_orbit[indices] = 1
        cloud_fraction_orbit[:] = cloud_mask_orbit[:]
        # Note opposite def as in original cloud...idx_cloud !=1, dx_no_cloud == 1?!)
        idx_cloud = np.where((cloud_mask_orbit >= fraction_minimum))
        idx_clear = np.where((cloud_mask_orbit < fraction_minimum))

        cloud_mask_orbit[idx_clear] = 0
        cloud_mask_orbit[idx_cloud] = 1
        _logger.debug('cloud percentage in orbit = {}'.format(np.sum(cloud_mask_orbit[idx_cloud]) / float(n_pixels)))

    t_end = time.perf_counter()
    _logger.debug("Time for interpolations: nearest total {}".format(t_end - t_start))
