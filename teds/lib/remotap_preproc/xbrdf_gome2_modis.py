...
class XbrdfGomeModis(object):
    ...
    def read_file(self):
        ...
        if 3 >= int(self.month) >= 1:
            self.brdf_filename = self.brdf_gome_modis_path + 'brdf_joost_jan.nc'
        elif 6 >= int(self.month) >= 4:
            self.brdf_filename = self.brdf_gome_modis_path + 'brdf_joost_apr.nc'
        elif 9 >= int(self.month) >= 7:
            self.brdf_filename = self.brdf_gome_modis_path + 'brdf_joost_july.nc'
        elif 12 >= int(self.month) >= 10:
            self.brdf_filename = self.brdf_gome_modis_path + 'brdf_joost_oct.nc'
        else:
        ...
        self.waves_gome2_bands = root_grp.variables['bands_gome2'][:]  # (waves_gome2) 21(wave)
        # Select data for GOME2
        self.xbrdf_wave_in = root_grp.variables['min_ler'][:][:][:]  # (xbrdf_wave) 21(wave)*180(lat)*360(lon)
        # Determine the shape
        self.nbands_GOME2, nlat_surf, nlon_surf = self.xbrdf_wave_in.shape

        ###################
        self.waves_combined_bands = root_grp.variables['bands_gome2'][:]  # (waves_gome2) 21(wave)
        self.waves_combined_bands = np.append(self.waves_combined_bands, root_grp.variables['bands_modis'][1])  # (waves_modis) 4(wave)
        self.waves_combined_bands = np.append(self.waves_combined_bands, root_grp.variables['bands_modis'][4:7])

        self.xbrdf_wave_combine_in = root_grp.variables['min_ler'][:][:][:]  # (xbrdf_wave_combine) 25(wave)*180(lat)*360(lon)
        ...
        self.xbrdf_wave_combine_in = np.append(self.xbrdf_wave_combine_in, xbrdf_wave_tmp, axis=0)
        ...
        self.xbrdf_rli_in[0, :, :] = root_grp.variables['brdf_LiSparse_band4'][:][:]  # (Xbrdf_rli[0,:,:]) 180(lat)*360(lon)
        self.xbrdf_rli_in[1, :, :] = root_grp.variables['brdf_RossThick_band4'][:][:]  # (Xbrdf_rli[1,:,:]) 180(lat)*360(lon)
    ...
    def collocate(self, julday_orbit, lat, lon, n_pixels, add_xbrdf_wave=True, add_xbdrf_wave_combine=True, add_xbrdf_rli=True):
        ...
        if add_xbrdf_rli:
            self.xbrdf_rli_orbit = interpolate_to_orbit(self.lat_orbit, self.lon_orbit, n_pixels, self.lats_surf,
                                                        self.lons_surf, self.xbrdf_rli_in, self.npar_rli)

        if add_xbrdf_wave:
            self.xbrdf_wave_orbit = interpolate_to_orbit(self.lat_orbit, self.lon_orbit, n_pixels, self.lats_surf,
                                                         self.lons_surf, self.xbrdf_wave_in, self.nbands_GOME2)

        if add_xbdrf_wave_combine:
            self.xbrdf_wave_combine_orbit = interpolate_to_orbit(self.lat_orbit, self.lon_orbit, n_pixels, self.lats_surf,
                                                                 self.lons_surf, self.xbrdf_wave_combine_in, len(self.waves_combined_bands))

    def write_output(self, output_dir, alt_orbit, julday_orbit, add_xbrdf_wave=True, add_xbdrf_wave_combine=True, add_xbrdf_rli=True):

        if add_xbrdf_rli:
            _logger.debug('now Write (orbital) xbrdf to ncdf')
            xbrdf_rli_file = output_dir + self.fname_xbrdf_rli_prefix + '.nc'
            self.write_to_nc_xbrdf(self.xbrdf_rli_orbit, self.lat_orbit, self.lon_orbit, self.n_pixels, xbrdf_rli_file)

        if add_xbrdf_wave:
            _logger.debug('now Write (orbital) xbrdf_wave (GOME2) to ncdf')
            xbrdf_file = output_dir + self.fname_xbrdf_wave_prefix + '.nc'

            self.write_to_nc_xbrdfwave(self.xbrdf_wave_orbit, self.lat_orbit, self.lon_orbit, self.n_pixels, self.nbands_GOME2,
                                       self.waves_gome2_bands, xbrdf_file)

        if add_xbdrf_wave_combine:
            _logger.debug('now Write (orbital) xbrdf_wave_combine (GOME2 and MODIS) to ncdf')
            xbrdf_combine_file = output_dir + self.fname_xbrdf_wave_combine_prefix + '.nc'
            self.write_to_nc_xbrdfwave_combine(self.xbrdf_wave_combine_orbit, self.lat_orbit, self.lon_orbit,
                                               self.n_pixels, len(self.waves_combined_bands), self.waves_combined_bands, xbrdf_combine_file)

    def write_to_nc_xbrdfwave(self, Xbrdf_wave_orbit, lat_orbit, lon_orbit, Npixels, Nbands_GOME2, waves_gome2, xbrdf_file):
        ...
        da.createVariable('Xbrdf_wave_GOME2', 'f4', ('Nbands_GOME2', 'Npixels_orbit'))
    ...
    def write_to_nc_xbrdfwave_combine(self, Xbrdf_wave_orbit, lat_orbit, lon_orbit, Npixels, Nbands_combine, waves_combine,
                                      xbrdf_file):
        da = Dataset(xbrdf_file, 'w', format='NETCDF4')
        ...
        da.createVariable('Xbrdf_wave_combine', 'f4', ('Nbands_combine', 'Npixels_orbit'))
    ...
    def write_to_nc_xbrdf(self, Xbrdf_rli_orbit, lat_orbit, lon_orbit, Npixels, xbrdf_rli_file):
    ...
    def write_to_group(self, group, start=0, end=None):
        ...
        if not self.combined_with_sentinel:
            ...
            group.variables['Xbrdf_wave_combine'][:, start:end] = self.xbrdf_wave_combine_orbit[:]

        group.variables['Lisparse'][start:end] = self.xbrdf_rli_orbit[0, :]
        group.variables['Rossthick'][start:end] = self.xbrdf_rli_orbit[1, :]
        group.variables['water_frac_inland'][start:end] = 0.0 * np.ones(self.n_pixels)
        group.variables['water_wind'][start:end] = 7.0 * np.ones(self.n_pixels)
        group.variables['snow_kernel_coef'][start:end] = 0.0 * np.ones(self.n_pixels)
