...
from .io import write_atmosphere_ref
...
def remove_geometry_margins(gm_filename: str,
                            geometry_ext: Geometry,
                            albedo: npt.NDArray[np.float64],
                            atm: Atmosphere) -> None:
    """Reduce data dimensions by removing the geometry margins.

    The GM generated two geometries: normal and an extended
    geometry. The latter is denser and contains points outside the
    target area so that convolutions could be performed
    correctly. This functions removes the margins from selected
    quantities but otherwise we proceed with the dense grid.
    ...
    sh_norm = geometry.lat.shape
    sh_ext = geometry_ext.lat.shape
    # Margins along and across track
    m_alt = int((sh_ext[0] - (sh_ext[0] // sh_norm[0]) * sh_norm[0]) / 2)
    m_act = int((sh_ext[1] - (sh_ext[1] // sh_norm[1]) * sh_norm[1]) / 2)
    n_alt, n_act = sh_ext
    reduce_alt_act_dimension(
        geometry_ext, albedo, atm, m_alt, n_alt-m_alt, m_act, n_act-m_act)
...
def carbon_radiation_scene_generation(config_user: dict) -> None:
    ...
    # anymore.
    remove_geometry_margins(
        config['io_files']['geometry'], geometry, albedo, atm)

    # Work with an ALT slice if requested by the user
    reduce_alt_act_dimension(
        geometry, albedo, atm, config['alt_beg'], config['alt_end'])

    write_atmosphere_ref(config['io_files']['atmosphere_ref'],
                         atm,
                         albedo,
                         geometry,
                         config['bin_alt'],
    ...
    if config['isrf']['enabled']:
        ...
        rad_output.solar_irradiance = kernel.convolve(sun)
    ...
    for i_alt in tqdm(range(n_alt)):
        ...
        if config['isrf']['enabled']:
            for i_act in range(n_act):
                rad_output.spectra[i_alt, i_act, :] = kernel.convolve(
                    radiance_lbl[i_act, :])
